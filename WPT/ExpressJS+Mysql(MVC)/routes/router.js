...
router.get("/delete/:pnum", function (req, resp) {
  connection.query(
    ...
    [req.params.pnum], //pid is received via path therefore req.params is used
    function (err, result) {
      if (err) {
        console.log(err);
        resp.status(500).send("<h3>no data found</h3>");
  ...
  connection.query(
    ...
    function (err, data) {
      if (err) {
        ...
        console.log(data);
        resp.render("edit-prod", { prod: data[0] }); //select query gives array which has only one product details therefore data[0] is selected
      }
    }
  );
});
